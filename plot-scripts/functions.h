...
// returns missing mass, based on reco particles
float missingMass(float ecm, const Vec_rp& in, float p_cutoff = 0.0f) {
    double px=0, py=0, pz=0, Evis=0;
    for (const auto& p : in) {
        const double pt = std::hypot(p.momentum.x, p.momentum.y);
        if (pt < p_cutoff) continue;
        px   += p.momentum.x;
        py   += p.momentum.y;
        pz   += p.momentum.z;
        Evis += p.energy;
    }
    const double Emiss = ecm - Evis;
    const double pxm = -px, pym = -py, pzm = -pz;
    const double mm2 = Emiss*Emiss - (pxm*pxm + pym*pym + pzm*pzm);
    return mm2 > 0.0 ? std::sqrt(mm2) : 0.0f;  // clamp small negatives from rounding
}
